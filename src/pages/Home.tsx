...
export default function HomePage() {
  ...
  return (
    <AppBarsWrapper>
      ...
      <Stack spacing={3} sx={{ alignItems: 'center', mt: '24px', mb: '24px', textAlign: 'center' }}>
        {isLoadingGroups ? (
          Array.from({ length: 3 }).map((_, index) => (
            <Card
              key={index}
              sx={{
                width: generateSeparateStyle('70%', '60%'),
                display: 'flex',
                flexDirection: 'row',
              }}
            >
          ...
          <>
            ...
            <Typography>
              Erstelle eine neue Gruppe, sodass du auf einen Klick ein neues Ereignisse für
              bestimmte Personen erstellen kannst.
            </Typography>
          ...
          <>
            <Typography variant="body1">
              Erstelle ein Ereignis für eine deiner Gruppen...
            </Typography>
            {groups.map((group) => (
              <GroupCard
                key={group.id}
